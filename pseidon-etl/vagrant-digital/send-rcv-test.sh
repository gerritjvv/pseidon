...
MYSQL_PWD=pseidon mysql -upseidon -e  \"insert into pseidon.pseidon_logs (log, format, output_format, log_group, enabled) values ('${TEST_TOPIC}', 'txt:ts=0;sep=tab', 'json', 'etl', 1) on duplicate key update enabled=1\" || exit -1
