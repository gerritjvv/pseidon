...
## avro registry
SRV1=$(vagrant ssh-config services1 | grep 'HostName' | awk '{print $2}' | tr '\n' ' ' | tr -d '[:space:]')
...
echo "Running mvn run send on client $SRV1 $BRK1,$BRK2,$BRK3 $TOPIC $THREADS $COUNT_PER_THREAD"

vagrant ssh client -c "cd /vagrant/pseidon-etl; mvn clojure:run -Dclojure.mainClass=pseidon_etl.pseidon_etl -Dclojure.args=\"send http://${SRV1}:8081 $BRK1,$BRK2,$BRK3 $TOPIC $THREADS $COUNT_PER_THREAD\" && sleep 10s"
